  ...
  async function handleGenerate(target: File): Promise<void> {
    ...
    try {
      ...
      for (const entry of FAVICON_SIZES) {
        ...
        done.push({ filename: entry.filename, size: entry.size, label: entry.label, url: URL.createObjectURL(blob), blob });
      }
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            <Button type="button" disabled={file === null || busy} onClick={() => (file === null ? undefined : void handleGenerate(file))}>
        ...
        <CardContent>
          {results.length === 0 ? (
            <p className="text-sm text-zinc-500">
              {busy ? "Drawing every size…" : "All six sizes previewed at their real dimensions will appear here."}
            </p>
          ) : (
            <div className="space-y-3">
              {results.map((result) => (
                <div key={result.filename} className="flex items-center gap-3 rounded-lg bg-zinc-50 p-3 dark:bg-zinc-900">
                  <span className="flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden rounded-md bg-white bg-[repeating-conic-gradient(#e5e7eb_0_25%,#fff_0_50%)] bg-[length:16px_16px]">
                    ...
                    <img src={result.url} alt={result.filename} width={result.size} height={result.size} loading="lazy" />
                  </span>
                  ...
                  <Button type="button" variant="outline" size="sm" onClick={() => downloadBlob(result.blob, result.filename)}>
              ...
              <div className="rounded-lg bg-zinc-950 p-3">
                <pre className="overflow-x-auto text-xs text-zinc-100">{html}</pre>
              </div>
