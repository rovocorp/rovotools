    ...
    try {
      ...
      for (const file of files) {
        ...
        const push = (blob: Blob, name: string): void => {
          done.push({ name, fromBytes: file.size, toBytes: blob.size, url: URL.createObjectURL(blob), blob });
        };
        if (mode === "target") {
          ...
          push(await encodeImage(img, width, height, "image/jpeg", best), replaceExtension(file.name, "jpg"));
          if (webp) {
            push(await encodeImage(img, width, height, "image/webp", best), replaceExtension(file.name, "webp"));
          }
        } else {
          push(await encodeImage(img, width, height, "image/jpeg", q), replaceExtension(file.name, "jpg"));
          if (webp) {
            push(await encodeImage(img, width, height, "image/webp", q), replaceExtension(file.name, "webp"));
          }
        }
      }
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          >
            ...
            <span className="text-xs text-zinc-500">JPG · PNG · WEBP · up to 20 at once</span>
          </button>
          <input
            ref={inputRef}
            type="file"
          ...
          {files.length > 0 ? (
            <p className="text-sm text-zinc-500">
              {files.length} file{files.length === 1 ? "" : "s"} selected · {formatBytes(originalBytes)} total
            </p>
          ) : null}
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex flex-wrap gap-2">
                {COMMON_TARGETS.map((target) => (
                  <Button key={target.kb} type="button" variant="outline" size="sm" onClick={() => setTargetKb(String(target.kb))}>
                    {target.label}
                  </Button>
                ))}
              </div>
              <p className="text-xs text-zinc-500">
                The compressor searches for the highest quality that still fits under this size. Very small targets on large images work far better with the 1920px cap on.
              </p>
            </div>
          )}
          <label className="flex cursor-pointer items-center gap-2 text-sm">
            <input type="checkbox" checked={webp} onChange={(event) => setWebp(event.target.checked)} className="h-4 w-4 rounded accent-indigo-600" />
            Convert output to WebP as well (usually 25–35% smaller than JPG)
          </label>
          <label className="flex cursor-pointer items-center gap-2 text-sm">
            <input type="checkbox" checked={capWidth} onChange={(event) => setCapWidth(event.target.checked)} className="h-4 w-4 rounded accent-indigo-600" />
          ...
          <div className="flex flex-wrap gap-2">
            <Button type="button" disabled={files.length === 0 || busy} onClick={() => void handleCompress()}>
        ...
        <CardContent>
          {results.length === 0 ? (
            ...
            <div className="space-y-3">
              ...
              {results.map((result) => (
                <div key={result.url} className="flex items-center gap-3 rounded-lg bg-zinc-50 p-3 dark:bg-zinc-900">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={result.url} alt={result.name} width={64} height={64} className="h-16 w-16 rounded-md object-cover" loading="lazy" />
                  ...
                  <Button type="button" variant="outline" size="sm" onClick={() => downloadBlob(result.blob, result.name)}>
