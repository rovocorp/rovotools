  ...
  try {
    ...
    const source =
      resolved.source === 'live'
        ? `Live rate via ${FX_API_ATTRIBUTION}, ${live?.date ?? ''}`
        : 'Approximate built-in rate';
...
export default function AdsenseCalculator(): React.ReactElement {
  ...
  const [rateNote, setRateNote] = useState(() => {
    ...
    return cached === null
      ? 'Approximate built-in rate'
      : `Cached rate via ${FX_API_ATTRIBUTION}, ${cached.date}`;
  });
    ...
    try {
      ...
      try {
        window.localStorage.setItem(
          CACHE_KEY,
          JSON.stringify({ date: fresh.date, rates: fresh.rates }),
        );
      } catch {
